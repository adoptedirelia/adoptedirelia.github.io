...
```
constrain <constrain name> <constrain condition>
```
...
# select data in more than 1 table
...
```
select sno
from s outer join sc on s.sno=sc.no
```
...
this command create a new table by selecting data from table.
...
> *where* means you can modify certain column.
...
**delete more than 2 tables is not allowed**

> *where* means you can delete certain data.
...
you can create a new user named `new_username` by the above command.

then accredit the user

```
grand <privilege>[,<privilege>] [on <database>.<table>] to <user_name>[,<user_name>] [with grant option];
...
this command create a trigger

```
drop trigger <trigger name>
```

this command delete a trigger

# pymysql

you can also use python to connect to mysql and execute some commands like inserting or selecting.
...
# run command with cursor
cursor.execute("show databases")

# data is the return of the command
data = cursor.fetchall()

# let database execute the command
db.commit()
